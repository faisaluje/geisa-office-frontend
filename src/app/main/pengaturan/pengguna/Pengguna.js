...
import { closeListPenggunaDialog, exitListPengguna, getListPengguna } from './store/tableSlice';
...
function Pengguna() {
  ...
  const { isLoading, props, params, isRefresh } = useSelector(({ pengguna }) => pengguna.table);

  React.useEffect(() => {
    if (params && isRefresh && props?.open) {
      dispatch(getListPengguna());
    }
  }, [dispatch, isRefresh, params, props]);

  React.useEffect(() => {
    return () => dispatch(exitListPengguna());
  }, [dispatch]);
