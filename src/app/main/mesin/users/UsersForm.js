...
import { refreshListPegawai } from 'app/main/pegawai/store/tableSlice';
...
function UsersForm() {
  ...
  const handleChangePegawai = (_evt, val) => {
    setForm({
      ...form,
      pegawai: val,
      name: dataMesin?.connectedMesin?.status === 'online' ? val?.nama || form.name : form.name
    });
  };
  ...
  return (
    <form noValidate onSubmit={handleSubmit} className="flex flex-col overflow-hidden">
        ...
        >
          ...
          <div className="flex mb-24">
            ...
            <TextField
              ...
              helperText={
                dataMesin?.connectedMesin?.status === 'offline' && 'Hanya bisa dirubah ketika terhubung dengan mesin'
              }
              InputProps={{
      ...
      <DialogActions>
        <Button variant="contained" color="secondary" onClick={handleSubmit} type="submit" disabled={!canBeSubmitted}>
          Simpan
        </Button>
      </DialogActions>
    </form>
