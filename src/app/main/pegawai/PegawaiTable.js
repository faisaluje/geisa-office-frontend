...
function PegawaiTable() {
  ...
  const { data, params, isLoading, isRefresh } = useSelector(({ pegawai }) => pegawai.table);
  ...
  return (
    <FuseScrollbars className="flex mt-8">
      <TableContainer component={Paper} elevation={3} className="w-full" style={{ height: 'calc(100vh - 238px)' }}>
            ...
            {rows?.length < 1 ? (
              ...
              rows.map((pegawai, idx) => (
                <TableRow hover key={idx}>
                  ...
                  <TableCell>
                    <Tooltip arrow classes={{ tooltip: 'text-14' }} title="Klik untuk melihat detil Pegawai">
                      <Typography
                        className={
                          pegawai.mesinUsers?.length > 0 ? 'text-blue hover:underline' : 'text-grey hover:underline'
                        }
                        role="button"
                  ...
                  <TableCell align="center">
                    {pegawai?.kehadiran?.masuk ? moment(pegawai.kehadiran.masuk).format('HH:mm') : '-'}
                  </TableCell>
                  <TableCell align="center">
                    {pegawai?.kehadiran?.pulang && pegawai?.kehadiran?.masuk !== pegawai.kehadiran.pulang
