...
import { closePasswordDialog, setPasswordForm, savePassword } from './store/passwordSlice';

function ChangePassword() {
  const dispatch = useDispatch();
  const { isLoading, props, data, isError, msg } = useSelector(({ profile }) => profile.password);
  ...
  const onSubmitPassword = values => {
    dispatch(setPasswordForm(values));
    dispatch(savePassword({ id: user.id, data: values }));
